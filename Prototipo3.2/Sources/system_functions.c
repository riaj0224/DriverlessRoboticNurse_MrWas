...
extern unsigned short duty_cycleA;							
...
void WiFi_setup(void)
{
	...
	//UART1_write("AT+CWJAP=\"INFINITUMACFE_2.4\",\"Yw3H794Xn8\"");//Connect to web
	UART1_read("OK");									//Read specific answer
	while(!(messageComplete));							//Message received confirmation
...
{                      
	...
	UART1_read("CONNECT");								//Read specific answer
	int p =10000;
	do{p--;}while(!(messageComplete) && (p));			//Message received confirmation
...
{                         
	...
	switch (instruction[0])
	{
		case 'a':
			...
			GPIOE_PCOR=(1<<4);								//Turn off output
			
			GPIOE_PCOR=(1<<5);								//Turn off output
			GPIOE_PCOR=(1<<30);								//Turn off output
			
			GPIOE_PSOR=(1<<3);								//Turn on output
			GPIOE_PCOR=(1<<4);								//Turn off output
			
			GPIOE_PSOR=(1<<5);								//Turn on output
			GPIOE_PCOR=(1<<30);								//Turn off output
			break;
		...
		case 'b':
			...
			GPIOE_PCOR=(1<<3);								//Turn off output
			GPIOE_PSOR=(1<<4);								//Turn on output
			
			GPIOE_PCOR=(1<<5);								//Turn off output
			GPIOE_PSOR=(1<<30);								//Turn on output
			break;
		...
		case 'l':
			...
			GPIOE_PSOR=(1<<3);								//Turn off output
			GPIOE_PCOR=(1<<4);								//Turn on output
		...
		case 'r':
			...
			GPIOE_PSOR=(1<<5);								//Turn off output
			GPIOE_PCOR=(1<<30);								//Turn on output
			break;
			
		case 'w':
			WiFi_send(id,"<!DOCTYPE html><html><head><title>ESP8266</title></head><body><p>Hello World</p></body></html>");
			break;	
			
		case 'd':
			raw_value = distancia & 0xFF ;
			data_requested [0] = (raw_value/1000) + 48 ;
			data_requested [1] = ((raw_value%1000)/100) + 48 ;
			data_requested [2] = (((raw_value%1000)%100)/10) + 48 ;
			data_requested [3] =(((raw_value%1000)%100)%10) + 48 ;
			concatenate2(string3,"Distance: ");
			concatenate2(string3,data_requested);
			concatenate2(string3," cm<br>");
			...
			data_requested [2] = (((raw_value%1000)%100)/10) + 48 ;
			data_requested [3] =(((raw_value%1000)%100)%10) + 48 ;
			concatenate2(string3,"Temperature: ");
			concatenate2(string3,data_requested);
			concatenate2(string3," °C<br>");
			

			concatenate2(string3,string4);
			WiFi_send(id, string3);
			break;
