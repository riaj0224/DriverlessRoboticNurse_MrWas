...
/*
	...
	PTE3        |GPIO(output)    |Direction Motor 1				|Polling               |
	PTE4        |GPIO(output)    |Direction Motor 1				|Polling               |
	PTE5        |GPIO(output)    |Direction Motor 2				|Polling               |
	PTE20		|TPM1_CH0        |Sending PWM signal to motor1  |Polling               |
	PTE22		|TPM2_CH0        |Sending PWM signal to motor2  |polling               |
	PTE30       |GPIO(output)    |Direction Motor 2				|Polling               |
*/
...
void PORT_init(void)
{
	...
	PORTE_PCR30 = (1<<8);								//Select 1st alternative: GPIO
	// Output
	GPIOE_PDDR |= (1<<2);								//Set as output
	
	GPIOE_PDDR |= (1<<3);								//Set as output
	GPIOE_PDDR |= (1<<4);								//Set as output
	GPIOE_PDDR |= (1<<5);								//Set as output
	GPIOE_PDDR |= (1<<30);								//Set as output
	// Turn off output
...
void PIT_init (void)
{
	...
	PIT_LDVAL0= 2000000;									//As the core clock is of 4MHz, 400k pulses equals 100 ms
